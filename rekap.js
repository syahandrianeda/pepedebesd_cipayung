        ...
        <table class="w3-table-all garis rekaptabelsekecamatan w3-tiny">
          <thead >
            <tr class="w3-light-grey">
              ...
              <th colspan="3"> < 6 tahun</th>
              <th colspan="3">  6 tahun</th>
              <th colspan="3">  7 tahun</th>
              <th colspan="3">  >= 7 tahun</th>
      ...
      .then(r => {
        ...
        urlscript = r.setting;
            ...
            for (i = 0; i < urlscript.length; i++) {
                ...
                <td>${rekapcpdblolos.filter(l => l.cpdb_jk == "L" && umur(l.cpdb_tanggal_lahir).tahun < 6).length}</td> <td>${rekapcpdblolos.filter(l => l.cpdb_jk == "P" && umur(l.cpdb_tanggal_lahir).tahun < 6).length}</td> <td>${rekapcpdblolos.filter(l => umur(l.cpdb_tanggal_lahir).tahun < 6).length}</td>
                <td>${rekapcpdblolos.filter(l => l.cpdb_jk == "L" && umur(l.cpdb_tanggal_lahir).tahun == 6).length}</td><td>${rekapcpdblolos.filter(l => l.cpdb_jk == "P" && umur(l.cpdb_tanggal_lahir).tahun == 6).length}</td><td>${rekapcpdblolos.filter(l => umur(l.cpdb_tanggal_lahir).tahun == 6).length}</td>
                <td>${rekapcpdblolos.filter(l => l.cpdb_jk == "L" && umur(l.cpdb_tanggal_lahir).tahun == 7).length}</td><td>${rekapcpdblolos.filter(l => l.cpdb_jk == "P" && umur(l.cpdb_tanggal_lahir).tahun == 7).length}</td><td>${rekapcpdblolos.filter(l => umur(l.cpdb_tanggal_lahir).tahun == 7).length}</td>
                <td>${rekapcpdblolos.filter(l => l.cpdb_jk == "L" && umur(l.cpdb_tanggal_lahir).tahun == 7).length}</td><td>${rekapcpdblolos.filter(l => l.cpdb_jk == "P" && umur(l.cpdb_tanggal_lahir).tahun > 7).length}</td> <td>${rekapcpdblolos.filter(l => umur(l.cpdb_tanggal_lahir).tahun > 7).length}</td>
                <td contenteditable="true">${rekapcpdblolos.filter(l => l.cpdb_jk == "L").length}</td><td contenteditable="true">${rekapcpdblolos.filter(l => l.cpdb_jk == "P").length}</td><td contenteditable="true">${rekapcpdblolos.length}</td>
...
const rekap_panggilsemuadataharian = async () => {
    //rekapsettingpagu:
    //rekapsettigprioritas:
    //rekappendaftar:
    //rekaploos;
    ...
    div.innerHTML = `
    <div class="w3-margin w3-center   w3-pale-red w3-round-large" style="margin:0 auto">
      <h2 class="">Rekapitulasi Pendaftar Harian</h2>
      <div class="tombolpengawas w3-margin">
        <button onclick="prntPengawsrekap()" class="w3-button w3-green w3-round-xxlarge">PRINT</button>
        <button onclick="xlPengawsrekap()" class="w3-button w3-blue w3-round-xxlarge">Export Xls</button>
      ...
      <div class="tabelrekappengawas w3-border w3-border-blue w3-round-large w3-margin w3-white" style="overflow-x: auto;">
        <h4 class="w3-center" contenteditable="true">Data Pendaftar Harian PPDB SDN Kecamatan Cipayung</h4>
        <h5 class="w3-center" contenteditable="true">TAHUN PELAJARAN 2022/2023</h5> 
        <table class="w3-table-all garis rekaptabelsekecamatan w3-tiny">
          <thead >
            <tr class="w3-light-grey">
              <th rowspan="2">No</th>
              <th rowspan="2">Nama Sekolah</th>
              <th colspan="5">Jumlah Pendaftar Harian</th>
              <th rowspan="2">Total</th>  
            </tr>
            <tr class="w3-light-grey">
              <th>Hari 1 <br> 4 Juli</th>
              <th>Hari 2 <br> 5 Juli</th>
              <th>Hari 3 <br> 6 Juli</th>
              <th>Hari 4 <br> 7 Juli</th>
              <th>Hari 5 <br> 8 Juli</th>
            </tr>
          </thead>
          <tbody>
            <tr>
      ...
      .then(r => {
        ...
        urlscript = r.setting;
            ...
            for (i = 0; i < urlscript.length; i++) {
                ...
                <td>${i + 1}</td>
                <td>${namasekolah}</td>
                <td contenteditable="true">${db_pendaftarrekap.filter(l => new Date(l.waktu_daftar).getMonth() == 6 && new Date(l.waktu_daftar).getDate() == 4 ).length}</td>
                <td contenteditable="true">${db_pendaftarrekap.filter(l => new Date(l.waktu_daftar).getMonth() == 6 && new Date(l.waktu_daftar).getDate() == 5 ).length}</td>
                <td contenteditable="true">${db_pendaftarrekap.filter(l => new Date(l.waktu_daftar).getMonth() == 6 && new Date(l.waktu_daftar).getDate() == 6 ).length}</td>
                <td contenteditable="true">${db_pendaftarrekap.filter(l => new Date(l.waktu_daftar).getMonth() == 6 && new Date(l.waktu_daftar).getDate() == 7 ).length}</td>
                <td contenteditable="true">${db_pendaftarrekap.filter(l => new Date(l.waktu_daftar).getMonth() == 6 && new Date(l.waktu_daftar).getDate() == 8 ).length}</td>
                <td contenteditable="true">${db_pendaftarrekap.length}</td>
                
               
            </tr>`;
                console.log(i)
                tbody.innerHTML = `<tr><td colspan="8" class="w3-center"><img src="/app/barloading.gif" alt="proses loading"> Memproses data ${namasekolah}</td></tr>`;
            };
